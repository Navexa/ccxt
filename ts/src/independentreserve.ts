...
export default class independentreserve extends Exchange {
    ...
    async fetchMyTrades (symbol: Str = undefined, since: Int = undefined, limit: Int = 50, params = {}) {
        ...
        try {
            ...
            for (let i = 0; i < brokerageFees.length; i++) {
                ...
                const correlationId = this.safeString (fee['info'], 'CorrelationId');
            ...
            for (let i = 0; i < trades.length; i++) {
                ...
                const tradeGuid = this.safeString (trade['info'], 'TradeGuid');
                const matchedFee = this.safeValue (feeMap, tradeGuid);
     ...
     */
    async fetchTransactions (code: Str = undefined, since: Int = undefined, limit: Int = 50, params = {}) {
        await this.loadMarkets ();
        let currency = undefined;
        const request: Dict = this.ordered ({
